...
This is the **internal genius layer** of IDEA Brand Coach: Trevor Bradford's book *What Captures The Heart Goes In The Cart* (2025) decomposed into an atomic, fully-traceable skill library. Generators and the MCP gateway consume these skills; this library only *holds* the knowledge.
...
## Structure rule
**No folder holds more than 5 children**, recursively — including the root. All content lives under [`framework/`](framework/) so the root stays at five entries (`README.md`, `_state.json`, `_coverage.md`, `_source/`, `framework/`). When a topic needs more than five atomic skills, it nests a subfolder.
