---
title: Does Dressing Up Your Confirmation Email Move Retention?
description: A beard-care founder added brand content to his confirmation email and wants proof. get_sequence_performance and compute_trust_gap_lift measure what changed.
date: 2026-05-25
category: Measure
...
Going forward, the habit worth keeping is treating every "I improved this email" moment as a before/after comparison instead of a one-time edit. `get_sequence_performance` before rolling a change wider gives Owen the actual delta on the specific step that changed; `compute_trust_gap_lift` tells him whether the targeted pillar moved, not just whether some downstream number wobbled. That discipline is the same one worth applying to [whether urgency messaging actually lifts conversion](/blog/does-urgency-messaging-lift-conversion/) or [testing subscribe-and-save messaging](/blog/subscribe-and-save-messaging-test/) rather than shipping either on instinct. It's also worth checking against [which trust element to test first](/blog/which-trust-element-to-test-first/) if you're planning several changes at once, and against [whether a welcome series that gets opens is actually driving repeat purchase](/blog/welcome-series-opens-but-no-repeat-purchase/) before assuming engagement equals retention.
...
Before you roll any "this reads better now" email change out to your full list, pull the specific step's own before/after with `get_sequence_performance` first. If you can't point to the metric that would tell you the change failed, you don't have a measured result yet — you have an opinion.
