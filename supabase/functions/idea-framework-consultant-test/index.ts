...
serve(async (req) => {
  ...
  try {
    ...
    if (authHeader) {
      ...
      console.log('[TEST] getUser result:', { hasUser: !!user, userId: user?.id, authError });

      if (user) {
        userId = user.id;
        console.log('[TEST] Authenticated user:', userId);

        // Ensure user has vector stores (create if first time)
        console.log("[TEST] Ensuring user KB exists...");
        ...
        if (!ensureKbResponse.ok) {
          ...
          console.error("[TEST] Failed to ensure user KB:", ensureKbResponse.status, errorText);
        } else {
          const kbResult = await ensureKbResponse.json();
          console.log("[TEST] User KB status:", kbResult.exists ? "already exists" : "created");
        }
      }
    }

    const { message, context } = await req.json();
    console.log('[TEST] IDEA Framework Consultant request:', {
      message,
    ...
    console.log('[TEST] User KB context length:', userKnowledgeContext.length);
...
- Prioritize and apply Trevor's methodology and frameworks
- Reference specific concepts from the IDEA framework
- Use Trevor's language and terminology where appropriate
- Ground your advice in the proven IDEA approach
...
Always encourage iterative refinement and ask clarifying questions when input lacks detail for optimal strategic guidance.`;
    ...
    // Add System KB context (Trevor's book) first
    if (systemKBContext) {
      userPrompt += `${systemKBContext}\n\n---\n\n`;
    }

    // Add User KB context (their specific brand data)
    ...
    if (systemKBContext || userKnowledgeContext) {
      userPrompt += `\n\nIMPORTANT: Use the knowledge base information above to provide personalized, specific advice. When System KB is provided, apply Trevor's IDEA Framework methodology. When User KB is provided, reference their actual brand inputs.`;
    }

    console.log('[TEST] Making OpenAI API request with model: gpt-4.1-2025-04-14');
    console.log('[TEST] Has system KB context:', !!systemKBContext);
    console.log('[TEST] Has user knowledge context:', !!userKnowledgeContext);
    ...
    try {
      ...
      console.log('[TEST] OpenAI API response status:', response.status);

      if (!response.ok) {
        const errorBody = await response.text();
        console.error('[TEST] OpenAI API error response:', errorBody);
        throw new Error(`OpenAI API error: ${response.status} - ${errorBody}`);
      }

      const data = await response.json();
      console.log('[TEST] OpenAI API response received successfully');

      const consultantResponse = data.choices[0].message.content;
      console.log('[TEST] IDEA Framework consultation completed successfully');

      return new Response(JSON.stringify({
        response: consultantResponse,
        _test_metadata: {
          systemKBContextLength: systemKBContext.length,
          userKBContextLength: userKnowledgeContext.length,
          hasSystemKB: !!systemKBContext,
          hasUserKB: !!userKnowledgeContext
        }
      }), {
        headers: {
          ...corsHeaders,
    ...
    console.error('[TEST] Error in idea-framework-consultant-test function:', error);
    return new Response(JSON.stringify({
