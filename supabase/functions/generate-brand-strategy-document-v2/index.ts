...
interface DocumentSection {
  ...
  batch: number; // For parallel generation grouping
  dependsOn?: string[]; // Section IDs this depends on
}
...
const DOCUMENT_SECTIONS: DocumentSection[] = [
  {
    ...
    order: 1,
    batch: 99, // Generated last — needs all other sections
...
If the user specified an archetype, explain how it manifests in the brand's communication and behaviour.`,
  },
  {
    ...
    order: 9,
    batch: 98, // After brand canvas sections (5-8)
    ...
    userDataFields: ['*'], // Synthesis of all canvas + avatar data
...
async function extractBrandName(
  ...
  try {
    const query = 'brand name company name product name what is the brand called';
    const embedding = await generateEmbedding(query);

    const [kbResult, docResult] = await Promise.all([
      supabaseClient.rpc('match_user_knowledge', {
        query_embedding: embedding,
        p_user_id: userId,
        match_count: 8,
    ...
    const allContent = [
      ...(kbResult.data || []).map((r: { content: string }) => r.content),
      ...(docResult.data || []).map((r: { content: string }) => r.content),
    ];

    // Add chat context
    if (chatInsights?.length) {
      const chatText = chatInsights.slice(0, 3)
        .map(c => c.excerpt.substring(0, 2000))
        .join('\n\n');
      allContent.push(chatText);
    }
    ...
    const response = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'system',
            content: `Extract the brand/company/product name from the context. Return ONLY the name (no quotes, no explanation). If not found, return: UNKNOWN. Do NOT return generic terms like "Your Brand" or framework terms like "IDEA Framework".`,
          },
          {
            role: 'user',
            content: `Find the brand name:\n\n${combined}`,
          },
        ],
        max_tokens: 30,
        temperature: 0,
    ...
    console.error('[extractBrandName] Error:', error);
    return null;
  }
}
...
async function generateSection(
  ...
  // From avatar_field_values (primary source)
  ...
  // From canvas/avatar/insights (legacy data sources)
  ...
  if (chatInsights.length > 0 && ['idea_overview', 'brand_story', 'implementation'].includes(section.id)) {
    chatContext = chatInsights.slice(0, 3).map(c => `### ${c.title}\n${c.excerpt}`).join('\n\n');
  }
  ...
  if (previousSections && section.dependsOn) {
    for (const depId of section.dependsOn) {
      if (previousSections[depId]) {
        // Include a brief summary (first 500 chars) of dependent sections
  ...
  const response = await fetchWithRetry('https://api.openai.com/v1/chat/completions', {
    ...
    body: JSON.stringify({
      ...
      messages: [
        {
          role: 'system',
          content: `You are editing a brand strategy document for coherence and consistency. Your job:
1. Ensure the brand name "${brandName}" is used correctly and consistently throughout
2. Fix any terminology inconsistencies between sections
3. Ensure cross-references between sections are accurate
4. Smooth section transitions where they feel abrupt
5. Fix any formatting issues (broken tables, inconsistent heading levels)

Rules:
- Do NOT add or remove content
- Do NOT change the strategic substance
- Do NOT add introductions, conclusions, or meta-commentary
- Preserve UK English spelling throughout
- Preserve the Trevor Bradford voice throughout
...
serve(async (req) => {
  ...
  try {
    ...
    console.log('[v2] Request received:', {
      companyName,
      fieldValueCount: Object.keys(avatarFieldValues).length,
      chatInsightsCount: chatInsights.length,
      hasSkillsVectorStore: !!skillsVectorStoreId,
    });

    // Extract brand name if needed
    let brandName = companyName || 'Your Brand';
    if ((brandName === 'Your Brand' || !brandName) && supabaseClient && userId) {
      const extracted = await extractBrandName(supabaseClient, userId, chatInsights);
      if (extracted) {
        brandName = extracted;
        console.log(`[v2] Extracted brand name: "${brandName}"`);
      }
    }
    ...
    // ========================================================================
    // SECTION-BY-SECTION GENERATION WITH PARALLEL BATCHING
    // ========================================================================

    const generatedSections: Record<string, string> = {};

    // Sort sections by batch number
    const sectionsByBatch = new Map<number, DocumentSection[]>();
    for (const section of DOCUMENT_SECTIONS) {
      const batch = section.batch;
      if (!sectionsByBatch.has(batch)) {
        sectionsByBatch.set(batch, []);
      }
      sectionsByBatch.get(batch)!.push(section);
    }
      ...
      const results = await Promise.all(
        batchSections.map(section =>
          generateSection(
            ...
            userId,
            generatedSections
          ).then(content => ({ id: section.id, content }))
        )
      );
    ...
    const orderedSections = DOCUMENT_SECTIONS
      .slice()
      .sort((a, b) => a.order - b.order);

    const documentTitle = `# ${brandName} — Brand Strategy Document\n\n*Generated using the IDEA Strategic Brand Framework™*\n*${generatedDate}*\n\n---\n`;

    let fullDocument = documentTitle;
