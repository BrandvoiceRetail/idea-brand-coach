...
- **Two feedback systems, one alive, one dead.** The Gen-3 `feedback_events` path (Positioning Statement "moment_1" only) is live (4 test rows). The older **Beta feedback suite** (widget + survey page + `beta_feedback`/`beta_comments`/`beta_testers` tables + `save-beta-*` edge fns) is fully built but **0 rows** — dead/unsurfaced.
- **Happy-path funnel is well instrumented; the corrective signal is not.** ~19 client funnel events exist, but the richest feedback — **users correcting/rejecting AI output** (field review accept/reject/edit, positioning statement backtrack/reroll) — is entirely local state, captured **nowhere**.
...
| Per-user records | Supabase SQL (`feedback_events`, `positioning_statements`, `diagnostic_submissions`, `chat_messages`, `avatar_field_values`, …) | RLS owner-scoped; query via service role. |
...
Landing → diagnostic → scorecard → auth → positioning statement → feedback. Counts/enums/IDs only (no PII/free-text).
...
| Positioning Statement | `positioning_statement_reveal_cta_shown`, `positioning_statement_reveal_requested`, `reviews_paste_shown`, `reviews_pasted`, `positioning_statement_options_shown`, `positioning_statement_picked` (PositioningStatementReveal.tsx / usePositioningStatementReveal.ts) | char_count, review_count_estimate, option_count, used_reviews, inference, chosen_index |
| Moment-1 feedback | `feedback_modal_opened`, `feedback_submitted`, `thank_you_viewed` (PositioningStatementFeedbackForm.tsx) | q1, q2 (yes/partial/no) |
| Errors | `llm_call_failed` ×7 (useChat.ts, useTrustGapInterpretation.ts, usePositioningStatementReveal.ts) | which_call, error_type |

- **Dead event:** `conversation_started` is defined in the union but **never fired** (no call site).
- **Verified live (203641, 30d):** only `positioning_statement_reveal_cta_shown`, `auth_completed`, `conversation_message_sent`, `auth_started`, `$pageview/$pageleave/$autocapture/$exception` have actually landed — all from 2026-06-16/17 test traffic. The rest haven't fired yet.
...
|---|---|---|---|
| **Moment-1 feedback** (Positioning Statement) | `PositioningStatementFeedbackForm.tsx` / `FeedbackMoment1.tsx` + `useFeedbackEvent.ts` | Q1 score-felt-right, Q2 positioning statement-felt-right, Q3 what's-off (free text), skip | PostHog (q1/q2 enums) + Supabase `feedback_events` (full, via `save-feedback-event`); **skip** is logged as `{skipped:true}` |
...
|---|---|---|---|
| `feedback_events` | Moment-tagged product feedback (only `moment_1`); `chosen_positioning_statement`, `scores`, `q1/q2/q3`, free text | 4 | **Yes — `posthog_distinct_id`** |
| `positioning_statements` | **`chosen_index`** (which option picked), `used_reviews`, `inference`, `all_options` | 9 | No |
...
`save-feedback-event` → `feedback_events` (the live path) · `save-beta-feedback`/`save-beta-comment`/`save-beta-tester` → beta tables (**dead**) · `diagnostic-interpretation(-evidence)` → `artifacts` · `reveal-positioning-statement` → `positioning_statements` · plus the generate/avatar/audit/competitor fns writing `artifacts`/`marketing_audits`.
...
2. **Positioning Statement dissatisfaction / churn** — ✅ done. `PositioningStatementReveal.tsx` fires `positioning_statement_reconsidered` ("See other options") and `positioning_statement_rerolled` ("Start over"/"Reveal again", with `from` = options|picked).
3. **Regeneration** — client-side, the only genuine redo surface is the positioning statement reveal-again/start-over (covered by `positioning_statement_rerolled` above) + field reject; the other "regenerate" hits are chat-title regen (low value). No separate artifact-regenerate UI exists to instrument.
...
### P2 — coverage & joins
4. **Other Supabase tables can't be joined to PostHog.** — ✅ DONE 2026-06-17 (deployed). Migration `20260617000000` added `posthog_distinct_id` to `positioning_statements`, `diagnostic_submissions`, `chat_sessions`; the client write paths (`SupabasePositioningStatementService`, `SupabaseDiagnosticService`, `ChatSessionService`) now stamp `getPostHogDistinctId()`. Populates on the next write of each.
