...
- **One parameterized analyzer**, not 26 agents: `(modality framework) × (touchpoint bindings from taxonomy) × (avatar + Signature)`, **output = IDEA Trust-Gap score per competitor**. ~7 modality profiles cover all 26 touchpoints (marketplace-listing, web/store-copy, visual/creative, email/lifecycle, social/content, reviews/social-proof, program/community).
