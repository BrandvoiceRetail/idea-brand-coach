...
**One-liner:** "I refined my avatar — now I have ~25 touchpoints to update and no way to see what's covered, stale, or off-strategy." Upload your assets, the agent maps them across your funnel, audits each against your avatar + Signature, and shows **current vs. desired** — then tracks lift when you redeploy refined messaging.
...
`upload assets → map to touchpoint → audit vs avatar+Signature → coverage visual (current vs desired) → redeploy refined messaging → record lift`
...
`psychographics.{values,fears,desires,triggers}`, `buying_behavior.{intent,decision_factors,shopping_style,price_consciousness}`, `voice_of_customer`, `demographics.*`. The messaging artifact is the coach's **Signature** (`signature`). v2 (`src/v2/domain/entities/Avatar.ts`) maps `painPoints↔fears`, `goals↔desires`.
...
1. **`brand_assets`** (new) — the asset-across-funnel ledger.
   `id, avatar_id (FK avatars), touchpoint_id (taxonomy), stage, applicable bool, source_ref (uploaded file / url), messaging_version_id (FK → Signature version), status ('covered'|'stale'|'missing'|'misaligned'), audit_result jsonb, created_at, updated_at`.
...
- `audit_asset(asset_id)` → runs the `audit_against` bindings vs avatar + current Signature → `status` + `audit_result`.
...
## Ownership boundary
Per the 2-server split: the **asset/test ledger lives on IV-OS** (it owns assets + tests); brand-coach contributes the **messaging version (Signature)** + the **assessment/audit**. Keep `brand_assets` / `brand_tests` on the IV-OS side; expose reads/writes through the brand-coach MCP gateway.
...
4. Sign off the `audit_against` bindings — which avatar/Signature fields *truly* drive each touchpoint's "on-strategy" verdict.
