# Positioning Statement Engine — Agent & Testing Context

Feature-local instructions for the **Positioning Statement reveal** (the "recognition moment").
For the shared test account and browser-QA setup, see `docs/TEST_ACCOUNT.md`
(pointed to from the top-level `CLAUDE.md`).

## What this feature is

The Positioning Statement names the deeper truth of what a customer is REALLY buying, in
Trevor's voice: "My customer isn't buying X. They're buying Y." It is the
retention / recognition moment.

**Acceptance bar (Trevor's):** a Positioning Statement works only if the user recognises it as
...
| Layer | Path | Notes |
|-------|------|-------|
| UI (dialog, cards, gold result) | `src/components/v2/positioning-statement/PositioningStatementReveal.tsx` | Self-contained `<Dialog>`. State machine: paste → loading → options → picked. Accepts `preloadedReviews`/`preloadedReviewCount` — when present, shows a "Using your N imported reviews" banner and prefills the textarea (manual paste/edit still works). |
| State / API call | `src/hooks/v2/usePositioningStatementReveal.ts` | Calls `supabase.functions.invoke('reveal-positioning-statement', …)`. Takes `{ initialReviews }` to seed (and re-seed on reset) the reviews state. Fires the `positioning_statement_picked` funnel event on pick. |
...
| Persistence | `src/services/SupabasePositioningStatementService.ts` (+ `interfaces/IPositioningStatementService.ts`) | Picking an option fire-and-forget saves `{positioning_statement_text, all_options, chosen_index, used_reviews, inference}` to the `positioning_statements` table (RLS owner-only). `BrandCoachV2` loads the latest pick on mount and shows it OUTSIDE the dialog as the "Your Positioning Statement: …" strip (`data-testid="saved-positioning statement-strip"`); a save failure never breaks the reveal moment. Added 2026-06-12 (Loop 05 Alpha bar: pick → reload → still shown). |
| Mount point | `src/pages/v2/BrandCoachV2.tsx` | `<PositioningStatementReveal messages={displayMessages} fieldValues={fieldValues} preloadedReviews={…} preloadedReviewCount={…} sessionId={currentSessionId} onPositioningStatementSaved={…} />` in the chat header. |
| Edge function | `supabase/functions/reveal-positioning-statement/index.ts` | Claude **Sonnet** (`claude-sonnet-4-6`), `verify_jwt: true`. No assistant prefill — Sonnet 4.6 rejects last-turn prefills (400); the handler unfences/reconstructs the JSON object instead. Needs the `ANTHROPIC_API_KEY` secret. Returns `{ options, usedReviews, inference }`. |
| Moment-1 feedback (Alpha) | `src/components/v2/positioning-statement/PositioningStatementFeedbackForm.tsx` | Renders inside the picked stage AFTER the surprise answer. Writes to `feedback_events` via the `save-feedback-event` edge fn. Every row carries `posthog_distinct_id` — THE JOIN KEY to the PostHog funnel. |
...
| Analytics | `src/lib/posthogClient.ts` | All funnel events (`positioning_statement_reveal_cta_shown`, `reviews_paste_shown/pasted`, `positioning_statement_reveal_requested/options_shown/picked`, `feedback_modal_opened/submitted`, `thank_you_viewed`, `llm_call_failed`). No-ops when `VITE_POSTHOG_KEY` is unset. Counts/booleans/IDs only — NEVER review text or conversation content in event props. |
...
If every claim in a Positioning Statement traces VERBATIM to the user's pasted/typed words, it
...
3. Click **Reveal Positioning Statement** (chat header). If the account has imported products
   ...
   otherwise paste InfinityVault reviews manually (sample below). Click **Reveal Positioning Statement**.
...
5. Pick one → dominant **gold-accent** result + "Did this surprise you?". The pick is
   persisted: expect a new `positioning_statements` row AND, after a page reload, the
   "Your Positioning Statement: …" strip under the chat header.
...
7. Verify the write: `SELECT moment, posthog_distinct_id, chosen_positioning_statement,
   q1_score_felt_right, q2_positioning_statement_felt_right, q3_whats_off FROM
...
emit `positioning_statement_reveal_cta_shown → reviews_paste_shown → reviews_pasted →
positioning_statement_reveal_requested → positioning_statement_options_shown → positioning_statement_picked →
...
`POST` to `https://ecdrxtbclxfpkknasmrw.supabase.co/functions/v1/reveal-positioning-statement`
