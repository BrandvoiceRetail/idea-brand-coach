...
| Edge function | `supabase/functions/reveal-signature/index.ts` | Claude **Sonnet** (`claude-sonnet-4-6`), `verify_jwt: true`, JSON output — no assistant prefill (Sonnet 4.6 rejects last-turn prefills with a 400). Needs the `ANTHROPIC_API_KEY` secret. Returns `{ options, usedReviews, inference }`. |
