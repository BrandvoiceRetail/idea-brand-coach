...
export function registerRunOnboardingTool(server: McpServer): void {
  server.registerTool(
    'run_onboarding',
    {
      ...
      description:
        'Start (or re-run) onboarding for the signed-in user. CALL THIS — do not ask the user to paste a prompt — whenever the user has not onboarded yet (no funnel data) OR asks to onboard / set up / re-run onboarding. Returns the user\'s current onboarding state (avatars, funnel pieces, whether any metrics exist) plus an ordered PLAYBOOK to execute now: read brand context, create the funnel pieces, check Windsor get_connectors (pull the FULL history each connected source allows at daily granularity, and prompt the user to enable registered-but-off connectors), ingest the metrics against each piece (mark "—" never fabricate), then run the Trust Gap and name the weakest piece. Idempotent: already-onboarded returns a lighter refresh path unless force=true. RLS-scoped; requires a Supabase bearer token.',
      inputSchema,
    },
    async ({ force }) => {
