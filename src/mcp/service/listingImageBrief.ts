...
export function buildListingImageBrief(input: ListingImageBriefInput): ListingImageBriefResult {
  ...
  return {
    ...
    instructions: [
      `1) Ground the set in the user's IDEA context for "${input.product}": the Decision Trigger${trigger ? ` (${trigger})` : ' (identify it first via identify_decision_trigger if unknown)'}, the Avatar 2.0 emotional core${input.avatarSummary ? '' : ' (pull from get_avatar / the avatar build)'}, and the Trust Gap pillar to close${input.trustGapSummary ? '' : ' (from run_trust_gap)'}.`,
      '2) For EACH slot above, compose a Skill-10 brief: (A) one-line context for the recipient, (B) image/shot direction + emotional register + the trust signal, (C) copy/overlay direction (gallery slots only), (D) where it goes. Lead the lifestyle slot with the empathetic line.',
      '3) Apply the execution routing: photoreal slots get a ready-to-paste image-gen prompt (photography/Midjourney/Genrupt, not Canva); infographic slots get copy + layout for Canva.',
      '4) Run the claim gate on any guarantee/claim. Keep all confirmed facts exact.',
      '5) Save the brief with log_asset (content_type "amazon"), and open the split-test with design_test (hypothesis = this set lifts CTR on the main + CVR on the gallery for this piece). Stamp asset_created → asset_live via update_test_milestone as the real images are produced and go live, then re-measure.',
    ],
    summary: `Amazon ${marketplace} listing image-set brief: ${LISTING_IMAGE_SLOTS.length} slots (main is white-bg product photography with no added text; gallery carries the IDEA work). ${trigger ? `Tuned to the ${trigger} trigger. ` : ''}Compose per-slot briefs grounded in the Avatar + Trust Gap + Decision Trigger, route photoreal slots to a real image engine (not Canva layout-gen), preserve all claims, and wire the split-test.`,
  };
}
