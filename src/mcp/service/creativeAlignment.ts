...
export const HIGGSFIELD_HANDOFF = {
  ...
  reference_discipline:
    "Build the REFERENCE KIT first, then storyboards, then video: upload the user's REAL product photo (media upload) and generate from it a product reference sheet (multi-view, all angles) — never from imagination; the brand logo; a packaging mock (needed for unboxing formats); and a character sheet for any recurring human, covering EVERY state the film shows (e.g. goggles on AND off), so the video model never has to guess. For a hard-surface product where the SAME item must stay identical across many angles/generations, prefer a 3D PRODUCT MODEL over a 2D reference sheet (see product_model) — geometry is then locked once, not re-guessed per generation. Run the platform's eligibility check on uploaded references before generating. When a prompt carries multiple references, @-tag WHICH reference each element means — untagged references get confused and waste generations. In every product-bearing generation, cite the reference as strict and list the product invariants (shape, colour, proportions, included components) that must not change.",
  product_model:
    "PRODUCT-FIDELITY UPGRADE (optional, for fidelity-critical or reused products): instead of asking the 2D image model to re-invent consistent angles every generation, build the product as a 3D model ONCE and render canonical angles from it. Feed 1–4 real product photos (more angles = better) to Higgsfield generate_3d → a GLB mesh → render a turntable / the canonical hero + detail angles as stills. Those stills become the 'approved still / strict reference' every downstream step already points at (the product_reveal orbit, the main-image hero, generateListingImage's reference_image_urls) — so geometry, proportions and included components stay identical across every image and every scene, collapsing the usual 'many iterations to match the real product' loop. Persist the GLB + renders via log_asset. HARD CAVEAT: image→3D reconstruction SMEARS small on-pack text and logos (it hallucinates unseen faces) — treat the 3D model as the GEOMETRY/shape reference only, and RE-APPLY logos, pack copy and fine text as a decal/composite or in the final image prompt from the real photo; never trust the reconstruction for legible text. Skip 3D for soft/organic goods or single-use shots where a photo reference already suffices.",
  draft_economy:
